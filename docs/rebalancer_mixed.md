...
> **Status: Experimental** - not the production default. See `docs/rebalancer_approaches_playbook.md` for activation policy.
...
Apples-to-apples on-chain EV+gas comparison against `Rebalancer.rebalance(...)` is summarized in `docs/rebalancer_approaches_playbook.md` and archived run details live at `docs/archive/rebalancer/rebalancer_vs_rebalancer_mixed_benchmark_2026-03-03.md`. Data is generated by `test/RebalancerAB.t.sol::test_rebalancer_vs_mixed_apples_to_apples_report`.
