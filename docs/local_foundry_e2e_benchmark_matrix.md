...
skip row rather than being omitted, so the output JSONL always has a consistent five rows per
scenario.

## Known Limitations

- The single-market 98-outcome case deploys synthetic full-range Uniswap V3 pools. The on-chain
  exact solver relies on a tick scan that can revert in that regime; this is emitted as
  `onchain_revert_full_range_tick_scan` rather than treated as a solver regression.
